...
Arena *arena_init(size_t capacity, const char *name) {
    ...
    if (name != NULL) {
        ...
        snprintf(arena->debug_name, sizeof("unnamed_arenaXXX"), "%s%03u", "unnamed_arena", arena_count);
    }
#endif
...
Arena *arena_create_subarena(Arena *parent, size_t capacity, const char *name) {
    ...
    Arena *subarena = (Arena *)arena_aligned_malloc(parent, sizeof(Arena), _Alignof(max_align_t));
    if (NULL == subarena) {
...
#ifndef NDEBUG
    if (name != NULL) {
        snprintf(subarena->debug_name, sizeof(subarena->debug_name), "%s", name);
    } else {
        snprintf(subarena->debug_name, sizeof("unnamed_subarenaXXX"), "%s%03u", "unnamed_subarena", subarena_count);
    }
#endif /* NDEBUG */
